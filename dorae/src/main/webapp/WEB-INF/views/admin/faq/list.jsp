	...
	<nav aria-label="Page navigation example">
	  <ul class="pagination justify-content-center">
	    <li class="page-item <c:if test="${page.startPage == 1 }">disabled</c:if>">
	      <button class="page-link" value="/dorae/admin/faq/list?page=${page.startPage - 1 }" onclick="goToPage(this)">&lt;</button>
	    </li>
	    <c:forEach begin="${page.startPage }" end="${page.endPage }" var="p">
			<li class="page-item">
				<button class="page-link <c:if test="${page.page eq p }">active</c:if>" value="/dorae/admin/faq/list?page=${p }" onclick="goToPage(this)">${p }</button>
			</li>
		</c:forEach>
	    <li class="page-item <c:if test="${page.endPage == page.lastPage}">disabled</c:if>">
	      <button class="page-link" value="/dorae/admin/faq/list?page=${page.endPage + 1 }" onclick="goToPage(this)">&gt;</button>
	    </li>
	  </ul>
	</nav>
